...
例えば、myApp1 アプリを {{site.data.keyword.Bluemix_notm}} にプッシュし、そのドメインを `mynewapp.mybluemix.net` に設定します。 次に、別の myApp2 アプリを同じスペースにプッシュし、その URL 経路の 1 つを `mynewapp.mybluemix.net` に設定します。 この経路は、この時点で両方のアプリにマップされています。
