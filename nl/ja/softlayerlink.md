...
前にインフラストラクチャー・リソースに対して[コントロール・ポータルで 2FA ](/docs/customer-portal/cpenable2fa.html#customerportal_2fa)を有効にした場合、その後で {{site.data.keyword.Bluemix_notm}} アカウント MFA 設定を有効にすると、コントロール・ポータルでセットアップした 2FA は MFA アカウント設定でオーバーライドされます。 これは、アカウント MFA 設定を優先して、購入した 2FA をコントロール・ポータルで無効にできることを意味します。 ただし、フェデレーテッド・ユーザーである場合は MFA は適用されないため、インフラストラクチャー・リソースのセキュリティーを確保するためにコントロール・ポータルの 2FA セットアップを保持することが推奨されます。
