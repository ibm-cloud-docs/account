...
您还必须具有与 IBM 标识或链接帐户标识关联的有效密码才能通过[客户门户网站](https://control.softlayer.com)登录。
 ...
 * 确保输入 IBM 标识的标准电子邮件地址。
 {: tsResolve}
 * 如果您是使用 SoftLayer 标识的 SoftLayer 用户，那么必须在您有权访问的每个帐户中的客户门户网站内切换到 IBM 标识认证，然后才能使用 IBM 标识认证进行登录。有关更多信息，请参阅[切换到 IBM 标识](/docs/account/softlayerlink.html)。
...
使用 IBM 标识登录时，显示了登录成功消息，但未返回到 [{{site.data.keyword.Bluemix_notm}} 控制台](https://console.{DomainName})或[客户门户网站](https://control.softlayer.com)。
{: tsSymptoms}

使用以下其中一种解决方案：
{: tsResolve}
 * 关闭浏览器，清除高速缓存和 cookie，然后重试登录。
 * 确保您是通过 [{{site.data.keyword.Bluemix_notm}} 控制台](https://console.{DomainName})或[客户门户网站](https://control.softlayer.com)登录的，而不是直接从 IBM 标识认证服务进行登录。
...
在注册 {{site.data.keyword.Bluemix_notm}} 轻量帐户后，您将收到确认电子邮件。必须单击确认电子邮件中的链接以完成注册过程。
{: tsCauses}
