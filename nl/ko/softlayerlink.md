...
또한 계정이 연결된 후에는 다음 변경사항을 참고하십시오.
  ...
  * 미국 달러(USD)로 된 송장 하나를 수신합니다. 기존 {{site.data.keyword.Bluemix_notm}} 계정을 보유하고 있는 경우, {{site.data.keyword.Bluemix_notm}}를 통한 인프라 리소스의 청구는 계정이 연결된 이후 시작되는 새 청구 주기에 적용됩니다. 자세한 정보는 [연결된 계정에 대한 통합된 청구](/docs/account/linking_accounts.html)를 참조하십시오.
  * {{site.data.keyword.Bluemix_notm}} 콘솔에서 인프라 리소스의 사용량을 모니터링할 수 있습니다.

일단 계정이 연결되면 해당 연결을 해제할 수 없습니다.
{: tip}

## 연결된 계정에서 다중 요소 인증 사용
{: #2fa}

연결된 계정이 있는 경우 ID 및 액세스 **설정** 페이지를 사용하여 계정에 대한 다중 요소 인증(MFA)을 사용으로 설정할 수 있습니다. 이는 일반적으로 이중 인증(2FA)이라고도 하며, 표준 필수 IBM ID 및 비밀번호 이상의 계정에 액세스하기 위해 보안 계층을 추가합니다. 계정에 대한 MFA는 연결된 {{site.data.keyword.Bluemix_notm}} 계정의 모든 리소스에 적용됩니다. 다중 인증이 계정에 사용 가능한 경우 계정에 추가된 모든 사용자에게도 적용됩니다.

다중 요소 인증은 IBM ID별로 수행되지 않습니다. 계정별로 수행됩니다. IBM ID가 여러 계정과 연관되어 있고, 계정 간에 전환할 때는 이중 인증이 필요한 다른 계정으로 전환할 때마다 ID를 확인해야 합니다. 이는 이전 계정과 새 계정이 모두 같은 이중 인증 메커니즘으로 구성된 경우에도 해당됩니다.

이전에 인프라 리소스에 대해 [제어 포털에서 2FA](/docs/customer-portal/cpenable2fa.html#customerportal_2fa)를 사용으로 설정한 후 {{site.data.keyword.Bluemix_notm}} 계정 MFA 설정을 사용으로 설정하는 경우 MFA 계정 설정은 제어 포털에서 설정한 2FA를 대체합니다. 이는 계정 MFA 설정을 위하여 제어 포털에서 구매한 2FA는 사용 안함으로 설정할 수 있음을 의미합니다. 그러나 연합 사용자인 경우 MFA가 적용되지 않으므로 인프라 리소스의 보안을 위해 2FA 제어 포털 설정을 유지하려고 할 수 있습니다.
