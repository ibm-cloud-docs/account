...
계정이 IBM ID 계정으로 전환되면 SoftLayer 계정과 {{site.data.keyword.Bluemix_notm}} 계정을 연결하여 결합된 IaaS(Infrastructure as a Service) 및 PaaS(Platform as a Service) 리소스를 사용할 수 있습니다. 그런 다음 단일 로그인을 통해 IaaS 리소스 및 PaaS 리소스에 액세스할 수 있습니다. 또한 계정을 연결하면 사용하는 모든 PaaS 및 IaaS 리소스에 대해 하나의 청구서만 발행됩니다. 소유한 계정을 연결하거나, 마스터 사용자인 경우 사용자 계정을 연결할 수 있습니다.
