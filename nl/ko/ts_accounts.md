...
Lite 계정을 사용 중인 경우에는 청구 가능 계정으로 업그레이드하여 추가 메모리를 확보할 수 있습니다. **관리 > 청구 및 사용량**으로 이동하고 **업그레이드**를 선택하십시오.
{: tsResolve}
...
예를 들어, myApp1 앱을 {{site.data.keyword.Bluemix_notm}}로 푸시하고 도메인을 `mynewapp.stage1.mybluemix.net`으로 설정하십시오. 그리고 다른 myApp2 앱을 동일한 영역으로 푸시하고 URL 라우트 중 하나를 `mynewapp.stage1.mybluemix.net`으로 설정하십시오. 이제 라우트가 두 앱 모두에 맵핑됩니다.
